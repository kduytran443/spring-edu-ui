...
function DeleteExerciseFileDialog({ fileId, id, button, fileName, reload = () => {} }) {
    ...
    const submit = () => {
        exerciseService.deleteFile(id, fileId).then((data) => {
            if (data.id) {
                setAlert(true);
                reload();
                setTimeout(() => {
...
export default DeleteExerciseFileDialog;
